...
import { X, CheckCircle, Sparkles } from "lucide-react"; // swapped icon
...
export default function ConnectPopup({ name, setIsOpen }) {
  ...
  const handleChange = (e) => {
    ...
    setFormData({
      ...formData,
      [e.target.name]:
        e.target.name === "phone" ? value.replace(/\D/g, "") : value,
    });
  };
          ...
          {!submitted ? (
            <>
              ...
              <p className="text-sm text-gray-600 mb-4">
                Speak directly with <span className="font-medium">{name}</span>.
              </p>
            ...
            <div className="text-center flex flex-col items-center">
              <CheckCircle className="w-10 h-10 text-green-600 mb-2" /> {/* Bigger success check */}
