...
function PropertyHeaderSection({
  ...
  <div className="w-full lg:w-[32rem]">
    ...
    <div className="flex lg:hidden gap-3 mt-3">
      ...
      >
        {property.images[2] ? (
          <img src={property.images[2]} alt="thumb" className="w-full h-full object-cover" />
        ) : (
      ...
      <div
        className="h-24 w-1/3 relative rounded-2xl overflow-hidden cursor-pointer"
        onClick={() =>
          setLightboxIdx(property.video ? property.images.length : property.images.length - 1)
        }
      >
        {property.images[3] ? (
          <img src={property.images[3]} alt="preview" className="w-full h-full object-cover" />
        ) : (
          <div className="w-full h-full bg-gray-300" />
        )}
        <div className="absolute inset-0 bg-black/60 text-white flex items-center justify-center text-center px-2 font-semibold text-xs">
          {property.video
  ...
  {/* Thumbnails for laptop (vertical) */}
