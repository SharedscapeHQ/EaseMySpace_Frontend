        ...
        <div className={`${finished ? "w-full" : "lg:w-1/2"} w-full flex flex-col`}>
          {!finished ? (
            <motion.div
              ...
              className="bg-gray-50 text-gray-900 rounded-2xl shadow-md flex flex-col gap-6 p-8"
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                {questions[step].options.map((opt, i) => (
                  <button
                    ...
                    className="bg-white hover:bg-gray-100 text-gray-800 py-4 rounded-xl shadow-md transition duration-200"
                  >
                    {opt}
                  </button>
                ))}
            ...
            <>
              <motion.h2 initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-3xl font-bold mb-6 w-full">
                Matching Properties
              </motion.h2>
              ...
              <div className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide">
                {filteredProperties.map((p, idx) => (
                  <Link
                    ...
                    className="min-w-[270px] max-w-[270px] group bg-white rounded-2xl border border-zinc-200 flex-shrink-0 overflow-hidden relative transition-all duration-300"
                  >
                    ...
                    <div className="p-4 flex flex-col gap-1 z-10 relative">
                      ...
                      <p className="text-zinc-800 font-bold text-xs lg:text-base whitespace-nowrap lg:group-hover:text-white transition-colors duration-300">
                        ₹ {Number(p.price).toLocaleString()}/mo
                      </p>

                      <p className="text-gray-600 text-xs lg:group-hover:text-white transition-colors duration-300">
                        {p.location}
                      </p>
                    </div>
                  </Link>
            ...
            <div className="text-center col-span-full">
              <p className="text-gray-600 mb-4">No properties match your criteria.</p>
              <Link
                to="/demand-form"
